...
import org.example.shrimpo.solphyte.events.GrappleNodesData;
...
public class GrappleStartC2SPacket {
    ...
    public static void handle(GrappleStartC2SPacket msg, Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            ...
            double hitR = Math.max(1.0, org.example.shrimpo.solphyte.Config.grNodeHitRadius);
            ...
            SolphyteNetwork.sendTo(player, new org.example.shrimpo.solphyte.network.packet.GrappleNodeS2CPacket(nodePos, GrappleNodesData.getLifetimeTicks()));
